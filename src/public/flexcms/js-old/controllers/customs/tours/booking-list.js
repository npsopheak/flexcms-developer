(function (){
	app.controller('TourBookingListingCtrl', function($scope, CoResource, 
	    ...
	    function loadData(callback, offset, limit){
			...
			CoResource.resources.TourBooking.list({
				'offset': (offset - 1) * limit || 0,
				'limit': limit || 10,
		    	'ignore-offset': 0,
		    	'search': $scope.search.query || '',
		    	'status': $scope.search.filter || '',
		    	'sorts': $scope.search.sort || 'name', // $scope.sort || '',
		    }, function(s) {
	  ...
	  //   	$rootScope.loading('show');
	    ...
	    $rootScope.$on('dataDriverAssigned', function (){
	    	startCalling();
	    });

		// Assign tour

	 	$scope.assignTourDriver = function(item, ev) {
    		$mdDialog.show({
				controller: 'AssignDriverDialogCtrl',
				templateUrl: '/templates/customs.tours.assign-driver',
  		...
  		$scope.edit = function($event){
  			if ($scope.selected.length == 1){
				$scope.assignTourDriver($scope.selected[0], $event);
  			}
  		};
