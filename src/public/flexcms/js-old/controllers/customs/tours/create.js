(function (){
	app.controller('TourCreateCtrl', function($scope, uiGmapGoogleMapApi,
		...
		$scope.data = {};
		$scope.mode = 'create';
		$scope.data = CoResource.resources.Tour
			.get({ tourId: $scope._id || 'NO_DATA' }, function (s){
				$scope.loading = false;
				...
				if (!s.result){
		        	...
		        	// $scope.data.phones = $scope.data.phone_numbers ? $scope.data.phone_numbers.join(', ') : '';
		        	// $scope.data.emails = $scope.data.email_addresses ? $scope.data.email_addresses.join(', ') : '';
    	...
    	$scope.chooseFile = function (filetmp, callback){
			...
			var url = $rootScope.remoteUrl + '/admin/tours/' + $scope.data._id + '/' + (filetmp.type == 'logo' ? 'logo' : 'galleries');
	    ...
	    $scope.submit = function (hideDialog){
	    	...
	    	if ($scope.mode === 'create'){
	    		// $scope.data.hash = $scope.hash;
	 			...
	 			var item = new CoResource.resources.Tour($scope.data);
	 				...
	 				if (!hideDialog){
	 					return $mdDialog.show(
			      			$mdDialog.alert()
						        ...
						        .content('Tour listing is successfully created')
						        .ariaLabel('Create Tour Listing')
						        .ok('Got it!')
						)
		                .then(function(answer) {
		                    $location.path('tours/' + $scope.data._id);
		                }, function() {
		                    $location.path('tours/' + $scope.data._id);
		                });
	 				}
 					...
 					if (!hideDialog){
	 					return $mdDialog.show(
			      			$mdDialog.alert()
						        .parent(angular.element(document.body))
						        .title('Create Tour Listing')
						        .content('There was an error while creating tour listing. ' + CoResource.textifyError(f.data))
						        .ariaLabel('Create Tour Listing')
						        .ok('Got it!')
					    );
	 				}
	    		...
	    		var tour = new CoResource.resources.Tour.get({
	 				tourId: $scope.data._id
	 			}, function(){
	 				tour = _.extend(tour, $scope.data);
	 				tour.$update({tourId: $scope.data._id}, function (s, h){
	 					...
	 					return $mdDialog.show(
			      			$mdDialog.alert()
						        .parent(angular.element(document.body))
						        .title('Update Tour Listing')
						        .content('Tour listing is successfully updated')
						        .ariaLabel('Update Tour Listing')
						        .ok('Got it!')
						    );
	 				}, function (e){
	 					$rootScope.loading('hide');
	 					// fail(CoResource.textifyError(e.data));
		      			...
		      			return $mdDialog.show(
			      			$mdDialog.alert()
						        .parent(angular.element(document.body))
						        .title('Error Update Tour Listing')
 					...
 					return $mdDialog.show(
		      			$mdDialog.alert()
					        .parent(angular.element(document.body))
					        .title('Update Tour Listing')
					        .content('There was an error while updating Tour listing. ' + CoResource.textifyError(e.data))
					        .ariaLabel('Update Tour Listing')
					        .ok('Got it!')
					    );
 				});
        ...
        $scope.updateCover = function (item, ev){
            ...
            var confirm = $mdDialog.confirm()
                ...
                .content('Are sure to do set this image as Tour cover?')
                .ariaLabel('Set Cover')
                .ok('Yes')
                .cancel('No')
                .targetEvent(ev);
            $mdDialog.show(confirm).then(function() {
                $rootScope.loading('show');
                CoResource.resources.Tour.setCover({
                	tourId: $scope.data._id
                }, {
		...
		$scope.delete = function ($event){
			if ($scope.data._id){
				...
				var confirm = $mdDialog.confirm()
	                .parent(angular.element(document.body))
	                .title('Remove tour?')
	                .content('Are sure to remove this tour? You cannot undo after you delete it')
	                .ariaLabel('Remove Tour')
	                .ok('Yes')
	                .cancel('No')
	                .targetEvent($event);
	            $mdDialog.show(confirm).then(function() {
	                $rootScope.loading('hide');
	                CoResource.resources.Tour.delete({
	                	tourId: $scope.data._id
	                }, {}, function (s){
	                	$rootScope.loading('hide');
						$location.path('/');
	                	...
	                	$rootScope.loading('hide');
	                	alert('Sorry, this Tour cannot be set due to some reason, please contact administrator for more information');
	                });
		...
		$scope.deletePhotoLogo = function ($event){
			if ($scope.data._id){
				...
				var confirm = $mdDialog.confirm()
	                .parent(angular.element(document.body))
	                .title('Remove tour logo?')
	                .content('Are sure to remove this tour logo? You cannot undo after you delete it')
	                .ariaLabel('Remove tour')
	                .ok('Yes')
	                .cancel('No')
	                .targetEvent($event);
	            $mdDialog.show(confirm).then(function() {
	                $rootScope.loading('show');
	                CoResource.resources.Tour.deleteLogo({
	                	tourId: $scope.data._id
	                }, {}, function (s){
	                	$rootScope.loading('hide');
	                	...
	                	$rootScope.loading('hide');
	                	alert('Sorry, this tour logo cannot be set due to some reason, please contact administrator for more information');
	                });
