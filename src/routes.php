...
Route::group(['prefix' => config('flexcms.app.route.prefix'), 'middleware' => ['web', 'csrf'], 'namespace' => 'FlexCMS\BasicCMS\Api'], function()
{
    ...
    Route::get('/login', ['middleware' => config('flexcms.app.route.middleware.guest'), 'as' => 'login', 'uses' => function () {
        return view('flexcms::dashboard.login');
    }]);

    Route::get('/browse-media', function () {
        return view('flexcms::dashboard.browse-media');
    });

    // Should implement in frontend
    // Route::post('/login', ['uses' => 'Auth\AuthController@login']);
    // Route::get('/signout', ['uses' => 'Auth\AuthController@getLogout']);
    // Route::post('/api/login', ['uses' => 'Auth\AuthController@apiLogin']);
    // Route::get('/api/signout', ['uses' => 'Auth\AuthController@getApiLogout']);
